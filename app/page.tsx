...
export default function Home() {
 ...
 return <>
  {session&&me?.profile?<Workspace key={workspaceVersion} me={me} session={session} run={run} busy={busy} onNotice={setNotice} onInfo={setInfo} onRefreshMe={async()=>setMe(await api('me'))} onLogout={()=>run(async()=>{const {error}=await browserDB()!.auth.signOut();if(error)throw error;setMe(null);})}/>:
  ...
  {session&&me?.profile?.role==='client'&&pendingCase&&<div className="pending-import" role="status"><p><strong>Tu borrador te estaba esperando.</strong><br/>Guarda el caso y sus archivos en esta cuenta para continuar.</p><button className="button" disabled={busy} onClick={()=>void importDraft()}>{busy?'Guardando…':'Guardar en mi cuenta'}<ArrowRight size={17}/></button></div>}
  {session&&me&&!me.profile&&<Modal title={role==='lawyer'?'Tu perfil profesional':pendingCase?'Guarda tu espacio':'Hagamos espacio para tu caso'} onClose={()=>run(async()=>{await browserDB()!.auth.signOut();})}><p className="muted">{pendingCase?'Solo necesitamos cómo quieres aparecer para guardar lo que preparaste.':'Solo necesitamos estos datos para empezar.'}</p><ProfileForm role={role} busy={busy} onSave={p=>run(async()=>{await saveProfile(p);setAuthMode('');})}/></Modal>}
  {authMode&&(!session||authMode==='update')&&<Modal title={authMode==='signup'?(role==='lawyer'?'Tu próximo caso empieza aquí':'Vamos a dar el siguiente paso'):authMode==='reset'?'Recupera tu acceso':authMode==='update'?'Elige una nueva contraseña':'Qué bueno verte de nuevo'} onClose={()=>setAuthMode('')}>
   ...
   {!browserDB()?<div className="notice-panel">Estamos preparando la apertura. El registro estará disponible al conectar los servicios de la plataforma.</div>:<>
    {googleReady&&(authMode==='signup'||authMode==='login')&&<button type="button" className="oauth-button" onClick={()=>void continueWithGoogle()} disabled={busy}><Chrome size={18}/> Continuar con Google</button>}
    {googleReady&&(authMode==='signup'||authMode==='login')&&<div className="auth-divider"><span>o continúa con tu correo</span></div>}
