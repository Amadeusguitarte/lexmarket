...
test('chat reads stay between the two participants; revocation closes realtime and browser writes remain forbidden',async()=>{
 ...
 await pg.exec('delete from messages;delete from notifications;');
 ...
 await pg.query(`update access_requests set state='requested' where lawyer_id=$1`,[lawyer]);await pg.exec('set role authenticated');assert.equal((await pg.query('select * from messages')).rows.length,1);assert.equal((await pg.query('select * from notifications where recipient_id<>auth.uid()')).rows.length,0);await assert.rejects(pg.exec("update notifications set read_at=now()"));await pg.exec('reset role');
